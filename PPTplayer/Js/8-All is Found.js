...
SceneA.src = "../images/background/8-AD.jpg";
SceneB.src = "../images/background/8-B.jpg";
SceneC.src = "../images/background/8-C.jpg";
...
function Initial() {
    ...
    for (N = 0; N < 165 * 3; N = N + 2) {
        ...
        FallenImg2.Random(0 + (N + Math.random() * 1) * 1000 / 3, 2, -250, 750, 0, 700,30,50,0.5,1);
        FallenImg2.Random(0 + (N + Math.random() * 1) * 1000 / 3, 2, 750, 1750, 0, 700,30,50,0.5,1);
        FloatImg3.Random(0 + (N + Math.random() * 1) * 1000 / 3, 2, 260, 1260, 420, 420, 25, 38, 0.5, 0.5);
    }
    Rotate = 0;
}
...
function AutoUpdate() {
        ...
        for (N = 0; N <FloatImg3.ms.length; N++) {
            ...
            if (delms > 1000) {
                ...
                switch (FallenImg2.pic[N] * 1) { // 陷阱:如果pic資料型態為字串，那case取數字就會出問題
                    case 1:
                        context.drawImage(image1, 0, 0, 641, 641, -C, -C, size, size); //畫出來囉~
                        break;
                    case 2:
                        context.drawImage(image2, 0, 0, 641, 641, -C, -C, size, size); //畫出來囉~
                        break;
                }
                if (BubbleKey4 == 1) {
                    context.beginPath();
    ...
    if (FallKey2 == 1) {
        ...
        for (N = 0; N < FallenImg2.ms.length; N++) {
            ...
            if (delms > 200) {
                ...
                WindX = WindX + ((mouse.x - 1500 / 2) / 750 - WindX) / 50;
                WindY = WindY + ((mouse.y - 820 / 2) / 410 - WindY) / 50;
                if (Scene == 'A') { WindX = 0.15; WindY = 0.1; }
                else if (Scene == 'B') { WindX = 0.3; WindY = 0; }
                else if (Scene == 'D') { WindX = -0.1; WindY = 0.05; }
    ...
    if (SceneMs > ms) {
        ...
        if (t < 1) {
            ...
            FallKey2 = 0;
            TheaKey3 = 0;
            if (PreScene == 'A' || PreScene == 'D' || PreScene == 'B') {
                FallKey2 = 1;
                FallMs = ms;
            }
            else if (PreScene == 'C') {
...
for (n = 0; n <= 3; n++) { 
    AnimeLyric.Build(n, 'oneByOne', 300, 'manual', 8000);
}
for (n = 4; n <= 4; n++) {
    AnimeLyric.Build(n, 'oneByOne', 300, 'manual', 5500);
}
for (n = 5; n <= 7; n++) {
    AnimeLyric.Build(n, 'oneByOne', 300, 'manual', 8000);
}
...
for (n = 9; n <= 12; n++) {
    AnimeLyric.Build(n, 'oneByOne', 300, 'manual', 7000);
}
for (n = 13; n <= 15; n++) {
    AnimeLyric.Build(n, 'oneByOne', 300, 'manual', 7000);
}
for (n = 16; n <= 17; n++) {
    AnimeLyric.Build(n, 'oneByOne', 300, 'manual', 13000);
}
//時間參數
...
function Start() {
    ...
    iframe.setAttribute('src', "https://u.pcloud.link/publink/show?code=XZjYwmXZGmo9Y8IvtA5Js2iOjLptoJsU2qCk");
