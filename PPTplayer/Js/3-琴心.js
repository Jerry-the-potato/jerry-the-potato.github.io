...
image1.src = "../images/pic/theather.png";
imageG.src = "../images/pic/theatherG.png";
imageP.src = "../images/pic/theatherP.png";
imageR.src = "../images/pic/theatherR.png";
...
var SceneE = new Image();
var SceneH = new Image();
StarrySky.src = "../images/background/3-BC-.jpg";
GroundTree.src = "../images/background/3-BC+.png";
SceneA.src = "../images/background/3-A.jpg";
SceneD.src = "../images/background/3-D.jpg";
SceneE.src = "../images/background/3-E.jpg";
SceneH.src = "../images/background/3-H.jpg";
...
function Initial() {
    ...
    for (N = 0; N < 165 * 3; N = N + 2) {
        ...
        FallenImg2.Random(0 + (N + Math.random() * 1) * 1000 / 3, 2, 50, 1500, 750, 900,30,50,0.5,1);
        FallenImg2.Random(0 + (N + Math.random() * 1) * 1000 / 3, 2, 50, 1500, 900, 950,30,50,0.5,1);
        FloatImg3.Random(0 + (N + Math.random() * 1) * 1000 / 3, 1, 760, 760, 420, 420, 25, 38, 1, 1);
    }
    Rotate = 0;
}
...
function AutoUpdate() {
    ...
    if (WindStatusX == 0) { //風速加速漸弱
        WindX = WindX / Math.pow(2, 1 / 100); // 最大值1，每 50偵(1500毫秒) 減半，最終趨於0
    }
    else if (WindX * WindStatusX < 0) { // (+,-) or (-, +)
        WindX = WindX + 0.01 * (1 - Math.abs(WindX)) * WindStatusX;
    }
    else if (WindX * WindStatusX < 0.5) {
        WindX = 1.02 * WindX + 0.01 * WindStatusX; // 1 or -1
    }
    else{ //0.5後要加速並漸慢
        WindX = WindX + (1 * WindStatusX - WindX) / 25 + 0.005 * WindStatusX;
    }
    if (WindStatusY == 0) { //風速加速漸弱
        WindY = WindY / Math.pow(2, 1 / 100); // 最大值1，每 50偵(1500毫秒) 減半，最終趨於0
    }
    else if (WindY * WindStatusY < 0) { // (+,-) or (-, +)
        WindY = WindY + 0.01 * (1 - Math.abs(WindY)) * WindStatusY;
    }
    else if (WindY * WindStatusY < 0.5) {
        WindY = 1.02 * WindY + 0.01 * WindStatusY; // 1 or -1
    }
    else { //0.5後要加速並漸慢
        WindY = WindY + (1 * WindStatusY - WindY) / 25 + 0.005 * WindStatusY;
    }
    WindX2 = WindX2 + ((mouse.x - 1500 / 2) / 750 - WindX2) / 50;
    WindY2 = WindY2 + ((mouse.y - 820 / 2) / 410 - WindY2) / 50;

    if (Scene == 'FG') {
        Rotate = Rotate - 2 / 10000;
    }
    else {
    ...
    if (Scene == 'A') {
        ...
        context.translate(760, 420);
        context.rotate(Rotate * 2 * Math.PI);
        context.drawImage(StarrySky, 0, 0, 1850, 1850, -165 - 760, -505 - 420, 1850, 1850); //畫出來囉~
    }
    else if (Scene == 'D') {
        context.drawImage(SceneD, 0, 0, 1520, 840, 0, 0, 1520, 840);
    }
    else if (Scene == 'E') {
        context.drawImage(SceneE, 0, 0, 1520, 840, 0, 0, 1520, 840);
    }
    else if (Scene == 'FG') {
        context.translate(760, 420);
        context.rotate(Rotate * 2 * Math.PI);
        ...
        context.drawImage(SceneH, 0, 0, 1520, 840, 0, 0, 1520, 840);
    }
    ...
    //背景遮罩
    if (Scene == 'BC') {
        context.drawImage(GroundTree, 0, 0, 1520, 840, 0, 0, 1520, 840);
    }
    ...
    if (TheaKey3 == 1) {
        ...
        for (N = 0; N <FloatImg3.ms.length; N++) {
            ...
            if (delms > 1000) {
                ...
                if (Scene == 'E' || Scene =='FG') { WindX = 0.05; WindY = 0.01; }
                ...
                switch (FallenImg2.pic[N] * 1) { // 陷阱:如果pic資料型態為字串，那case取數字就會出問題
                    case 1:
                        context.drawImage(image1, 0, 0, 800, 800, -C, -C, size, size); //畫出來囉~
                        break;
                    case 2:
                        context.drawImage(imageP, 0, 0, 400, 400, -C, -C, size, size); //畫出來囉~
                        break;
                    case 3:
                        context.drawImage(imageR, 0, 0, 400, 400, -C, -C, size, size); //畫出來囉~
                        break;
                    case 4:
                        context.drawImage(imageG, 0, 0, 400, 400, -C, -C, size, size); //畫出來囉~
                        break;
                }
                if (BubbleKey4 == 1) {
                    context.beginPath();
    ...
    if (FallKey2 == 1) {
        ...
        for (N = 0; N < FallenImg2.ms.length; N++) {
            ...
            if (delms > 200) {
                ...
                size = FallenImg2.size[N] * (1 - Math.abs(delms - 100) / 120);
                Vt = (delms + 100) / 200 * Math.PI; // 速度 Vel = 0.05

                Ix = FallenImg2.x[N] * 1 - FallenImg2.Vx[N] * FallenImg2.V[N] / Vel * 2 * Math.sin((1 - Math.abs(FallenImg2.Vx[N])) * FallenImg2.Vt[N] * 1 + Vt); // 對Vx積分
                Iy = FallenImg2.y[N] * 1 + FallenImg2.Vy[N] * FallenImg2.V[N] / Vel * 3 * Math.cos((1 - Math.abs(FallenImg2.Vy[N])) * FallenImg2.Vt[N] * 1 + Vt); //Vy積分
    ...
    if (SceneMs > ms) {
        ...
        if (t < 1) {
            ...
            FallKey2 = 0;
            TheaKey3 = 0;
            if (PreScene == 'D' || PreScene == 'H') {
                FallKey2 = 1;
                FallMs = ms;
            }
            else if (PreScene == 'E' || PreScene == 'FG') {
                TheaKey3 = 1;
                FloatMs = ms;
            }
            ...
            context.beginPath();
            context.rect(0, 0, 1520, 840);
            context.fillStyle = "rgba(0, 0, 0, " + (2 - t) + ")";
            context.fill();
        }
    }
...
for (n = 0; n <= 3; n++) {
    AnimeLyric.Build(n, 'oneByOne', 400, 'manual', 9500);
}
for (n = 4; n <= 4; n++) {
    AnimeLyric.Build(n, 'oneByOne', 360, 'manual', 8000);
}
for (n = 5; n <= 7; n++) {
    AnimeLyric.Build(n, 'oneByOne', 200, 'manual', 7000, 'NONE');
}
for (n = 8; n <= 8; n++) {
    AnimeLyric.Build(n, 'oneByOne', 460, 'manual', 9600, 'OUT');
}
for (n = 9; n <= 11; n++) {
    AnimeLyric.Build(n, 'oneByOne', 230, 'manual', 8500, 'NONE');
}
for (n = 12; n <= 12; n++) {
    AnimeLyric.Build(n, 'oneByOne', 130, 'manual', 2000, 'OUT');
}
for (n = 13; n <= 16; n++) {
    AnimeLyric.Build(n, 'oneByOne', 500, 'manual', 9000, 'OUT');
}
//間奏
for (n = 16; n <= 18; n++) {
    AnimeLyric.Build(n, 'oneByOne', 400, 'manual', 9500);
...
for (n = 29; n <= 32; n++) {
    AnimeLyric.Build(n, 'oneByOne', 500, 'manual', 9000);
}
for (n = 33; n <= 33; n++) {
    AnimeLyric.Build(n, 'oneByOne', 2000, 'manual', 26000, 'OUT');
}
//時間參數
...
function Start() {
    ...
    iframe.setAttribute('src', "https://u.pcloud.link/publink/show?code=XZ1bwmXZVMgaLGJhiaJTVvC06PIu14xTS6QV");
    ...
    var contextT = canvasUnseen.getContext("2d");
        ...
        window.addEventListener('keyup', function (e) {
            KeyLock = 0;
            if (e.key == 'w' || e.key == 'W' || WindStatusY == -1) {
                WindStatusY = 0;
            }
            else if (e.key == 's' || e.key == 'S' || WindStatusY == 1) {
                WindStatusY = 0;
            }
            else if (e.key == 'a' || e.key == 'A' || WindStatusX == -1) {
                WindStatusX = 0;
            }
            else if (e.key == 'd' || e.key == 'D' || WindStatusX == 1) {
                WindStatusX = 0;
            }
        }, false);
}
