...
animeObject.prototype.NextFrame = function(){
    ...
    // 檢查是否與滑鼠碰撞
    let distance2p = Math.pow(this.pointX - myMouse.pointX, 2) + Math.pow(this.pointY - myMouse.pointY, 2);
...
animeObject.prototype.Falling = function(dT){
    ...
    const popSize = 0.3;
    this.sizeNow = WIDTH * this.size * (popSize + (1 - popSize) * this.GetSize(dT, this.lifeTime));
}
